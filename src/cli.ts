...
type Result = Awaited<ReturnType<typeof import("./sanitizer").InputSanitizer.prototype.sanitize>>;
...
if (has("-i")) {
	...
	const go = () =>
		rl.question("> ", async (q: string) => {
			...
			if (q) {
				print(await sanitizer.sanitize(q, { threshold, generateRefusal: has("--refusal") }));
			}
			go();
		});
	go();
	...
	const sanitizer = await createSanitizer(scope);
	const qs = (await Bun.file(get("--batch") as string).text()).split("\n").filter(Boolean);
	for (const q of qs) {
		const r = await sanitizer.sanitize(q.trim(), { threshold, generateRefusal: has("--refusal") });
		console.log(`[${r.decision}] ${q.slice(0, 60)}`);
	}
} else if (query) {
	const sanitizer = await createSanitizer(scope);
	print(await sanitizer.sanitize(query, { threshold, generateRefusal: has("--refusal") }));
} else {
	console.log(HELP);
}
